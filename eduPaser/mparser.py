...
class ProfileParser():
    def __init__(self,employee,text=None,max_line=32,min_char=2,lines=None, set_attr_hook=None):
        self.text = text
...
def employee_filter(aca_name, aca_url, url, name):
    ...
    if not url:
        logger.warning("url null in employee_filter")
        #return None,None
    if len(name) != 0:
        name = ''.join(name.split())
    # 太长的不识别为名称
